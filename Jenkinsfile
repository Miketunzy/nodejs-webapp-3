pipeline {
    ...
    environment {
    ACR_NAME = "miketunzyacr"
    registyUrl = "miketunzyacr.azurecr.io"
    IMAGE_NAME = "nodejswebapp"
    IMAGE_TAG = "v1.0.0"
    registryCredential  = "miketunzyacr"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/Miketunzy/nodejs-webapp-2.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
